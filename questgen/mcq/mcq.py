...
def sense2vec_get_words(word, s2v):
    ...
    for each_word in most_similar:
        ...
        if append_word_processed not in compare_list and word_preprocessed not in append_word_processed \
...
def get_options(answer, s2v):
    ...
    try:
        destructors = sense2vec_get_words(answer, s2v)
        if len(destructors) > 0:
            print(" Sense2vec_destructors successful for word : ", answer)
            return destructors, "sense2vec"
    except:
        print(" Sense2vec_destructors failed for word : ", answer)

    return destructors, "None"
...
def get_nouns_multipart_title(text):
    out = []
...
def get_keywords(nlp, text, max_keywords, s2v, fdist, normalized_levenshtein, no_of_sentences):
    ...
    keywords = get_nouns_multipart_title(text)
    ...
    for answer in total_phrases_filtered:
        if answer not in answers and mcq_available(answer, s2v):
